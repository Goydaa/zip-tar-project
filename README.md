...
- Создаем ZIP-архивы из любой папки 
- Создаем TAR.GZ архивы (сжатые) 
...
r - чтение существующего архива 
w - создание нового архива (перезаписывает существующий) 
a - добавление файлов в существующий архив (только для ZIP) 
x - создание нового архива (если файл есть - ошибка) 
...
О: ZIP сжимает каждый файл отдельно. TAR.GZ сначала объединяет все файлы в один TAR-архив, потом сжимает его целиком. TAR.GZ лучше сжимает много маленьких файлов. 
...
В: Как прочитать файл из архива без распаковки? 
О: content = zipf.read('file.txt') - читает файл прямо в память 
